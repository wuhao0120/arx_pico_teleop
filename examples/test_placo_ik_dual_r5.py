#!/usr/bin/env python3
"""
Offline Placo IK test for dual_R5a.urdf — no hardware or VR needed.

Tests the Placo pipeline used by teleop_arx.py:
  1. Load dual_R5a.urdf, set up solver + frame tasks + joints regularization
...
Note: dual_R5a.urdf uses revolute joints (not continuous), so joint angles are
      stored directly in state.q, not as cos/sin pairs.

Usage:
    conda run -n ur_data python test_placo_ik_dual_r5.py
"""
...
# ── Config ──────────────────────────────────────────────────────────────────
URDF_PATH = "assets/r5_urdf/dual_R5a.urdf"
SERVO_DT = 0.017
...
def build_q_index_map(robot):
    """Build mapping from joint name → state.q index(es)."""
    q_indices = {}
    joint_is_continuous = {}
    ...
    return q_indices, joint_is_continuous


def set_joint_angle(robot, q_indices, joint_is_continuous, joint_name, angle):
    """Set a joint angle, handling both continuous and revolute joints."""
    idxs = q_indices[joint_name]
    if joint_is_continuous[joint_name]:
        robot.state.q[idxs[0]] = math.cos(angle)
        robot.state.q[idxs[1]] = math.sin(angle)
    else:
        robot.state.q[idxs[0]] = angle


def get_joint_angle(robot, q_indices, joint_is_continuous, joint_name):
    """Read a joint angle, handling both continuous and revolute joints."""
    idxs = q_indices[joint_name]
    if joint_is_continuous[joint_name]:
        return math.atan2(robot.state.q[idxs[1]], robot.state.q[idxs[0]])
    else:
        return robot.state.q[idxs[0]]


def set_all_joints(robot, q_indices, joint_is_continuous, joints_dict):
    """Set multiple joint angles and update kinematics."""
    for name, val in joints_dict.items():
        set_joint_angle(robot, q_indices, joint_is_continuous, name, val)
    robot.update_kinematics()
...
def setup_solver(robot, q_indices, joint_is_continuous):
    ...
    for jn in robot.joint_names():
        if jn not in arm_joint_names:
            if joint_is_continuous[jn]:
                non_arm_joints[jn] = 1.0
            else:
                non_arm_joints[jn] = q0[q_indices[jn][0]]
...
def main():
    ...
    q_indices, joint_is_continuous = build_q_index_map(robot)
    print(f"  Joints: {len(robot.joint_names())}  |  state.q: {len(robot.state.q)}")
    print(f"  q_indices sample: left_joint1 -> q[{q_indices['left_joint1']}]")
    print(f"  Continuous joints: {sum(joint_is_continuous.values())}")
    print(f"  Revolute joints: {len([j for j in joint_is_continuous if not joint_is_continuous[j]])}")

    # ── 2. Joint set/get roundtrip ──────────────────────────────────────────
    print_header("Test 1: Joint set/get roundtrip")

    set_all_joints(robot, q_indices, joint_is_continuous, TEST_JOINTS)
    all_match = True
    for jn, target_val in TEST_JOINTS.items():
        actual_val = get_joint_angle(robot, q_indices, joint_is_continuous, jn)
    ...
    for arm_name, arm_cfg in ARMS.items():
        ...
        q_readback = [get_joint_angle(robot, q_indices, joint_is_continuous, jn) for jn in arm_cfg["joint_names"]]
        print(f"\n  {arm_name}:")
    ...
    solver, effector_task = setup_solver(robot, q_indices, joint_is_continuous)
    ...
    for delta_label, delta_xyz in position_deltas:
        ...
        T_init = robot.get_T_world_frame("left_link6")
        ...
        T_actual = robot.get_T_world_frame("left_link6")
        pos_err, rot_err = pose_error(T_actual, T_target)
    ...
    for delta_label, delta_rot in orientation_deltas:
        ...
        T_init = robot.get_T_world_frame("right_link6")
        init_xyz = T_init[:3, 3].copy()
        init_quat = tf.quaternion_from_matrix(T_init)
        ...
        T_actual = robot.get_T_world_frame("right_link6")
        pos_err, rot_err = pose_error(T_actual, T_target)
    ...
    for jn in robot.joint_names():
        if jn not in arm_joint_names:
            initial_non_arm[jn] = get_joint_angle(robot, q_indices, joint_is_continuous, jn)
    ...
    for jn, init_val in initial_non_arm.items():
        val = get_joint_angle(robot, q_indices, joint_is_continuous, jn)
